...
class UrlCollector:
    ...
    def collect_urls_from_database(self, limit=10):
        """从数据库中获取待抓取的URL并收集"""
        try:
            # 当使用--from-database参数时，获取随机URL而不是按顺序排列的URL
            pending_urls = []
            for i in range(limit):
                random_url = self.db.get_random_pending_url()
                if random_url:
                    pending_urls.append(random_url)
                else:
                    break
            ...
            print(f"从数据库随机获取了 {len(filtered_urls)} 个待抓取URL (过滤了 {len(urls) - len(filtered_urls)} 个屏蔽URL)")
